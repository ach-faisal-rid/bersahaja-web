...
const form = useForm({
    nama_bacaan: props.bacaan?.nama_bacaan ?? '',
    bacaan_arab: props.bacaan?.bacaan_arab ?? '',
    transliterasi: props.bacaan?.transliterasi ?? '',
    terjemahan: props.bacaan?.terjemahan ?? '',
    deskripsi: props.bacaan?.deskripsi ?? '',
    kategori: props.bacaan?.kategori ?? '',
});

const submit = () => {
    if (!props.bacaan?.id) return;
    form.put(`/bacaan/${props.bacaan.id}`);
};
</script>

<template>
    <Head title="Edit Bacaan" />
