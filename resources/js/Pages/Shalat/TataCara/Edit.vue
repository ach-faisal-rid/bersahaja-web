...
const form = useForm({
    nama_langkah: props.tataCara?.nama_langkah ?? '',
    urutan: props.tataCara?.urutan ?? '',
    deskripsi: props.tataCara?.deskripsi ?? '',
    gerakan_id: props.tataCara?.gerakan_id ?? '',
    bacaan_arab: props.tataCara?.bacaan_arab ?? '',
    transliterasi: props.tataCara?.transliterasi ?? '',
    terjemahan: props.tataCara?.terjemahan ?? '',
});

const submit = () => {
    if (!props.tataCara?.id) return;
    form.put(`/tata-cara/${props.tataCara.id}`);
};
</script>

<template>
    <Head title="Edit Tata Cara" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800 dark:text-gray-200">
                Edit Tata Cara Shalat
            </h2>
        </template>
        ...
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                <p class="text-sm text-gray-600 dark:text-gray-300">
                    Perbarui data tata cara shalat.
                </p>
                <Link href="/tata-cara">
