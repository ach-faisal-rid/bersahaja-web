...
const rows = computed(() => {
    if (Array.isArray(props.tataCara)) {
        return props.tataCara;
    }
    return props.tataCara?.data ?? [];
});
...
const destroy = (id) => {
    if (!id) return;
    if (window.confirm('Hapus tata cara ini?')) {
        router.delete(`/tata-cara/${id}`);
    }
};
        ...
        <div class="space-y-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <p class="text-sm text-gray-600 dark:text-gray-300">
                    Kelola tata cara pelaksanaan shalat.
                </p>
                ...
                <div class="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:items-center">
                    ...
                    <Link v-if="canManage" href="/tata-cara/create">
                ...
                <table class="w-full table-auto">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        <tr v-if="rows.length === 0">
                            <td :colspan="canManage ? 7 : 6" class="px-4 py-12 text-center text-sm text-gray-500 dark:text-gray-400">
                                Tidak ada data.
                            </td>
                        </tr>
                    </tbody>
            ...
            >
                <div class="text-sm text-gray-600 dark:text-gray-300">
                    Menampilkan
                    <span class="font-semibold">{{ props.tataCara?.from ?? 0 }}</span>
                    hingga
                    <span class="font-semibold">{{ props.tataCara?.to ?? 0 }}</span>
                    dari
                    <span class="font-semibold">{{ props.tataCara?.total ?? 0 }}</span>
                    hasil
                </div>
                <div class="flex gap-2">
                    <Link
                        v-for="link in props.tataCara?.links"
                        v-show="link.url"
                        :key="link.label"
                        :href="link.url || '#'"
