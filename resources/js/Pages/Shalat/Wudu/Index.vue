...
const rows = computed(() => {
    if (Array.isArray(props.wudus)) {
        return props.wudus;
    }
    return props.wudus?.data ?? [];
});
...
const destroy = (id) => {
    if (!id) return;
    if (window.confirm('Hapus langkah wudu ini?')) {
        router.delete(`/wudu/${id}`);
    }
};
        ...
        <div class="space-y-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <p class="text-sm text-gray-600 dark:text-gray-300">
                    Kelola langkah-langkah dan tata cara berwudu secara berurutan.
                </p>
                ...
                <div class="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:items-center">
                    ...
                    <Link v-if="canManage" href="/wudu/create">
                ...
                <table class="w-full table-auto">
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        >
                            ...
                            <td class="px-4 py-3 text-sm font-semibold text-gray-900 dark:text-gray-50">
                                {{ item.urutan }}
                            </td>
                            <td class="px-4 py-3 text-sm font-medium text-gray-900 dark:text-gray-50">
                                <Link :href="`/wudu/${item.id}`" class="hover:underline">
                                    {{ item.nama_langkah }}
                                </Link>
                            </td>
                            ...
                            <td v-if="canManage" class="px-4 py-3 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <Link :href="`/wudu/${item.id}`">
                                        <SecondaryButton>Lihat</SecondaryButton>
                                    </Link>
                                    <Link :href="`/wudu/${item.id}/edit`">
            ...
            >
                <div class="text-sm text-gray-600 dark:text-gray-300">
                    Menampilkan
                    <span class="font-semibold">{{ props.wudus?.from ?? 0 }}</span>
                    hingga
                    <span class="font-semibold">{{ props.wudus?.to ?? 0 }}</span>
                    dari
                    <span class="font-semibold">{{ props.wudus?.total ?? 0 }}</span>
                    hasil
                </div>
                <div class="flex gap-2">
                    <Link
                        v-for="link in props.wudus?.links"
                        v-show="link.url"
                        :key="link.label"
                        :href="link.url || '#'"
