...
const rows = computed(() => {
    if (Array.isArray(props.gerakans)) {
        return props.gerakans;
    }
    return props.gerakans?.data ?? [];
});
...
const destroy = (id) => {
    if (!id) return;
    if (window.confirm('Hapus gerakan shalat ini?')) {
        router.delete(`/gerakan-shalat/${id}`);
    }
};
        ...
        <div class="space-y-6">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                <p class="text-sm text-gray-600 dark:text-gray-300">
                    Kelola data gerakan-gerakan dalam pelaksanaan shalat.
                </p>
                ...
                <div class="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:items-center">
                    ...
                    <Link v-if="canManage" href="/gerakan-shalat/create">
                ...
                <table class="w-full table-auto">
                    <thead class="border-b border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-900">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-center text-sm font-semibold text-gray-900 dark:text-gray-50">
                                Lihat
                            </th>
                            <th
                                v-if="canManage"
                    ...
                    <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        >
                            ...
                            <td class="px-4 py-3 text-sm">
                                <span
                                    :class="{
                                        'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300': item.kategori === 'Wajib',
                                        'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300': item.kategori === 'Sunnah',
                                        'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300': item.kategori === 'makruh',
                                        'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300': item.kategori === 'haram',
                                    }"
                        ...
                        <tr v-if="rows.length === 0">
                            <td :colspan="canManage ? 8 : 7" class="px-4 py-12 text-center text-sm text-gray-500 dark:text-gray-400">
                                Tidak ada data.
                            </td>
                        </tr>
                    </tbody>
            ...
            >
                <div class="text-sm text-gray-600 dark:text-gray-300">
                    Menampilkan
                    <span class="font-semibold">{{ props.gerakans?.from ?? 0 }}</span>
                    hingga
                    <span class="font-semibold">{{ props.gerakans?.to ?? 0 }}</span>
                    dari
                    <span class="font-semibold">{{ props.gerakans?.total ?? 0 }}</span>
                    hasil
                </div>
                <div class="flex gap-2">
                    <Link
                        v-for="link in props.gerakans?.links"
                        v-show="link.url"
                        :key="link.label"
                        :href="link.url || '#'"
