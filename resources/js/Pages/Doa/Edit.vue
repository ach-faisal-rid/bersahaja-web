...
// Format fetched_at for datetime-local input
...
const form = useForm({
    repository_id: '',
    kategori_id: props.doa?.kategori_id ?? '',
    judul: props.doa?.judul ?? '',
    teks_arab: props.doa?.teks_arab ?? '',
    transliterasi: props.doa?.transliterasi ?? '',
    terjemahan: props.doa?.terjemahan ?? '',
    description: props.doa?.description ?? '',
    is_active: props.doa?.is_active ?? true,
    source: props.doa?.source ?? '',
    fetched_at: formatFetchedAt(props.doa?.fetched_at),
    hadist_sources: formatHadistSources(props.doa?.hadist_sources),
});
