...
// Format hadist sources from relationship
const formatHadistSources = (sources) => {
...
const form = useForm({
    judul: props.hadist?.judul ?? '',
    teks_arab: props.hadist?.teks_arab ?? '',
    transliterasi: props.hadist?.transliterasi ?? '',
    terjemah: props.hadist?.terjemah ?? '',
    kategori_id: props.hadist?.kategori_id ?? '',
    audio_url: props.hadist?.audio_url ?? '',
    gambar_url: props.hadist?.gambar_url ?? '',
    hadist_sources: formatHadistSources(props.hadist?.hadist_sources),
});
...
const submit = () => {
    if (!props.hadist?.id) return;
    form.put(`/hadists/${props.hadist.id}`);
};
</script>

<template>
    <Head title="Edit Hadist" />
